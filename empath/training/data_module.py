...
Handles data loading and preprocessing for training Empath LoRA adapters.
...
class EmpathTrainingDataset(Dataset):
    """Dataset for Empath LoRA training from raw audio.
...
class EmpathDataModule(LightningDataModule if LIGHTNING_AVAILABLE else object):
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == 'fit' or stage is None:
            if self.val_split > 0 and len(self.samples) > 1:
                ...
                self.train_dataset = EmpathTrainingDataset(
                    train_samples, self.dit_handler, self.max_duration
                )
                self.val_dataset = EmpathTrainingDataset(
