"""
LoRA Injection Utilities for Empath
...
from empath.training.configs import LoRAConfig
...
def _safe_enable_input_require_grads(self):
    ...
    orig_enable_input_require_grads = getattr(
        self, "_empath_orig_enable_input_require_grads", None
    )
    ...
    try:
        if orig_enable_input_require_grads is not None:
            ...
            result = None
        try:
            self._empath_input_grads_hook_enabled = True
        except Exception:
            logger.debug(
                "Failed to set _empath_input_grads_hook_enabled", exc_info=True
            )
        return result
    except NotImplementedError:
        try:
            self._empath_input_grads_hook_enabled = False
        except Exception:
            logger.debug(
                "Failed to set _empath_input_grads_hook_enabled", exc_info=True
            )
        if not getattr(self, "_empath_input_grads_warning_emitted", False):
            logger.info(
                "Skipping enable_input_require_grads for decoder: "
                "get_input_embeddings is not implemented (expected for DiT)"
            )
            try:
                self._empath_input_grads_warning_emitted = True
            except Exception:
                logger.debug(
                    "Failed to set _empath_input_grads_warning_emitted", exc_info=True
                )
        return None
...
def get_dit_target_modules(model) -> List[str]:
    ...
    Args:
        model: The EmpathConditionGenerationModel
...
def inject_lora_into_dit(
    ...
    if hasattr(decoder, "enable_input_require_grads"):
        ...
        decoder._empath_orig_enable_input_require_grads = orig
