"""
LoRA Trainer for Empath

Lightning Fabric-based trainer for LoRA fine-tuning of Empath DiT decoder.
...
from empath.training.lora_utils import check_peft_available
...
from empath.training.path_safety import safe_path
    ...
    for mod in _iter_module_wrappers(decoder):
        ...
        if hasattr(mod, "enable_input_require_grads"):
            try:
                ...
                hook_enabled = bool(
                    getattr(mod, "_empath_input_grads_hook_enabled", False)
                )
...
class LoRATrainer:
    """High-level trainer for Empath LoRA fine-tuning.
...
class LoKRTrainer:
    """High-level trainer for Empath LoKr fine-tuning."""
