...
try:
    # When executed as a module: `python -m empath.empath_v15_pipeline`
    from .handler import EmpathHandler
    from .llm_inference import LLMHandler
    from .dataset_handler import DatasetHandler
    from .ui.gradio import create_gradio_interface
    from empath.ui.gradio.i18n import get_i18n, available_languages_info
    ...
    # When executed as a script: `python empath/empath_v15_pipeline.py`
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    if project_root not in sys.path:
        sys.path.insert(0, project_root)
    from empath.handler import EmpathHandler
    from empath.llm_inference import LLMHandler
    from empath.dataset_handler import DatasetHandler
    from empath.ui.gradio import create_gradio_interface
    from empath.ui.gradio.i18n import get_i18n, available_languages_info
...
def create_demo(init_params=None, language="en"):
    ...
    if (
        ...
        dit_handler = EmpathHandler()  # DiT handler
    ...
    # Inject custom glassmorphism Theme CSS
    theme_file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "custom_theme.css")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Gradio Demo for Empath V1.5",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Main model path (e.g., 'empath-v15-turbo')",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="5Hz LM model path (e.g., 'empath-5Hz-lm-0.6B')",
    )
    parser.add_argument(
        "--backend",
        type=str,
    ...
    if args.enable_api:
        ...
        if args.config_path is None:
            args.config_path = os.environ.get("EMPATH_CONFIG_PATH")
        if args.lm_model_path is None:
            args.lm_model_path = os.environ.get("EMPATH_LM_MODEL_PATH")
        if os.environ.get("EMPATH_LM_BACKEND"):
            args.backend = os.environ.get("EMPATH_LM_BACKEND")
    ...
    if args.service_mode:
        ...
        if args.config_path is None:
            args.config_path = os.environ.get(
                "SERVICE_MODE_DIT_MODEL", "empath-v15-turbo-fix-inst-shift-dynamic"
            )
        # Default LM model for service mode (from env or fallback)
        if args.lm_model_path is None:
            args.lm_model_path = os.environ.get(
                "SERVICE_MODE_LM_MODEL", "empath-5Hz-lm-1.7B-v4-fix"
            )
    ...
    try:
        ...
        if args.init_service:
            ...
            if args.config_path is None:
                ...
                if available_models:
                    args.config_path = (
                        "empath-v15-turbo"
                        if "empath-v15-turbo" in available_models
        ...
        if args.enable_api:
            ...
            from empath.ui.gradio.api.api_routes import setup_api_routes
