...
# modeling_empath_v15_turbo.py using pure MLX operations for optimal
...
class MLXAttention(nn.Module):
    """Multi-head attention with QK-RMSNorm for the Empath DiT.
...
class MLXDiTDecoder(nn.Module):
    """Native MLX implementation of EmpathDiTModel (the diffusion transformer decoder).

    Mirrors the PyTorch ``EmpathDiTModel`` class exactly:
    ...
    def from_config(cls, config) -> "MLXDiTDecoder":
        """Construct from an EmpathConfig (transformers PretrainedConfig)."""
        return cls(
