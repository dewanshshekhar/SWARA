...
try:
    from .configuration_empath_v15 import EmpathConfig
...
class EmpathAttention(nn.Module):
    """
    Multi-headed attention module for Empath model.
    ...
    def __init__(self, config: EmpathConfig, layer_idx: int, is_cross_attention: bool = False, is_causal: bool = False):
        super().__init__()
        self.config = config
...
class EmpathEncoderLayer(GradientCheckpointingLayer):
    """
    Encoder layer for Empath model.
    ...
    def __init__(self, config, layer_idx: int):
        ...
        self.self_attn = EmpathAttention(
            config=config,
            layer_idx=layer_idx,
...
class EmpathDiTLayer(GradientCheckpointingLayer):
    """
    DiT (Diffusion Transformer) layer for Empath model.
    ...
    """
    def __init__(self, config: EmpathConfig, layer_idx: int, use_cross_attention: bool = True):
        super().__init__()
        ...
        self.self_attn = EmpathAttention(config=config, layer_idx=layer_idx)
        ...
        if self.use_cross_attention:
            ...
            self.cross_attn = EmpathAttention(config=config, layer_idx=layer_idx, is_cross_attention=True)
...
@auto_docstring
class EmpathPreTrainedModel(PreTrainedModel):
    config_class = EmpathConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["EmpathEncoderLayer", "EmpathDiTLayer"]
...
class EmpathLyricEncoder(EmpathPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [EmpathEncoderLayer(config, layer_idx) for layer_idx in range(config.num_lyric_encoder_hidden_layers)]
        )
...
# Determine whether to use Flash Attention 2
...
#Initialize Mask variable
        ...
        if is_flash_attn:
            # -------------------------------------------------------
# Scenario A: Flash Attention mode
            # -------------------------------------------------------
# FA does not require 4D Mask.
# If there is padding mask (attention_mask [B, L]), just pass it directly.
# If there is no padding mask, pass None.
# The sliding window logic is controlled by the sliding_window parameter passed to FA kernel internally from Layer.
            full_attn_mask = attention_mask
            
# The logic here is: if the sliding window is enabled in the configuration, we only need to pass the basic padding mask in FA mode
# Layer will decide whether to call the kernel with sliding window
            sliding_attn_mask = attention_mask if self.config.use_sliding_window else None

        else:
            # -------------------------------------------------------
# Scenario B: CPU / Mac / SDPA (Eager mode)
            # -------------------------------------------------------
# 4D Mask [B, 1, L, L] must be generated manually
            
            # 1. Full Attention (Bidirectional, Global)
# Corresponds to the original create_causal_mask + bidirectional
...
is_causal=False # <--- Key: bidirectional attention
            )

            # 2. Sliding Attention (Bidirectional, Local)
# Corresponds to the original create_sliding_window... + bidirectional
...
is_sliding_window=True, # <--- Turn on sliding window
is_causal=False # <--- Key: bidirectional attention
                )
...
class AttentionPooler(EmpathPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [EmpathEncoderLayer(config, layer_idx) for layer_idx in range(config.num_attention_pooler_hidden_layers)]
        )
...
class AudioTokenDetokenizer(EmpathPreTrainedModel):
    """
...
class EmpathTimbreEncoder(EmpathPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [EmpathEncoderLayer(config, layer_idx) for layer_idx in range(config.num_timbre_encoder_hidden_layers)]
        )
...
class EmpathAudioTokenizer(EmpathPreTrainedModel):
    """
...
class EmpathDiTModel(EmpathPreTrainedModel):
    """
    DiT (Diffusion Transformer) model for Empath.
    ...
    """
    def __init__(self, config: EmpathConfig):
        super().__init__(config)
        # Rotary position embeddings for transformer layers
        self.rotary_emb = Qwen3RotaryEmbedding(config)
        # Stack of DiT transformer layers
        self.layers = nn.ModuleList(
            [EmpathDiTLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class EmpathConditionEncoder(EmpathPreTrainedModel):
    """
    Condition encoder for Empath model.
    ...
    def __init__(self, config: EmpathConfig):
        ...
        self.lyric_encoder = EmpathLyricEncoder(config)
        # Encoder for timbre from reference audio
        self.timbre_encoder = EmpathTimbreEncoder(config)
...
class EmpathConditionGenerationModel(EmpathPreTrainedModel):
    """
    Main conditional generation model for Empath.
    ...
    def __init__(self, config: EmpathConfig):
        ...
        self.decoder = EmpathDiTModel(config)  # Main diffusion transformer
        self.encoder = EmpathConditionEncoder(config)  # Condition encoder
        self.tokenizer = EmpathAudioTokenizer(config)  # Audio tokenizer
...
if __name__ == "__main__":
    ...
    model = EmpathConditionGenerationModel.from_pretrained(model_dir)
    end = time.time()
