...
class EmpathConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`EmpathModel`]. It is used to instantiate an
    Empath model according to the specified arguments, defining the model architecture.
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 64003):
            Vocabulary size of the Empath model. Defines the number of different tokens that can be represented by the
    ...
    ```python
    >>> from empath.models import EmpathConfig

    >>> # Initializing an Empath configuration
    >>> configuration = EmpathConfig()

    >>> # Initializing a model from the configuration
    >>> model = EmpathConditionGenerationModel(configuration)
