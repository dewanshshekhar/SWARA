...
def load_batch_management_module(*, is_windows: bool = False) -> Tuple[Any, Dict[str, Any]]:
    ...
    empath_pkg = types.ModuleType("empath")
    ui_pkg = types.ModuleType("empath.ui")
    gradio_pkg = types.ModuleType("empath.ui.gradio")
    events_pkg = types.ModuleType("empath.ui.gradio.events")
    results_pkg = types.ModuleType("empath.ui.gradio.events.results")

    results_dir = Path(__file__).resolve().parent
    events_dir = results_dir.parent
    gradio_dir = events_dir.parent
    ui_dir = gradio_dir.parent
    empath_dir = ui_dir.parent

    empath_pkg.__path__ = [str(empath_dir)]
    ...
    modules = {
        ...
        "empath": empath_pkg,
        "empath.ui": ui_pkg,
        "empath.ui.gradio": gradio_pkg,
        "empath.ui.gradio.i18n": types.SimpleNamespace(t=_translate),
        "empath.ui.gradio.events": events_pkg,
        "empath.ui.gradio.events.results": results_pkg,
        "empath.ui.gradio.events.results.generation_info": types.SimpleNamespace(IS_WINDOWS=is_windows),
        "empath.ui.gradio.events.results.generation_progress": types.SimpleNamespace(
            generate_with_progress=_default_generate_with_progress
        ),
        "empath.ui.gradio.events.results.batch_queue": types.SimpleNamespace(
    ...
    empath_pkg.ui = ui_pkg
    ui_pkg.gradio = gradio_pkg
    gradio_pkg.events = events_pkg
    events_pkg.results = results_pkg

    for module_name in list(sys.modules):
        if module_name.startswith("empath.ui.gradio.events.results.batch_management"):
