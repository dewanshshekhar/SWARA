...
Add API endpoints compatible with api_server.py and CustomEmpath to Gradio application
"""
import json
import os
import random
...
def _wrap_response(data: Any, code: int = 200, error: Optional[str] = None) -> Dict[str, Any]:
    """Wrap response data in standard format compatible with CustomEmpath."""
    return {
        "data": data,
        "code": code,
        "error": error,
...
async def release_task(request: Request, authorization: Optional[str] = Header(None)):
    ...
    from empath.inference import generate_music, GenerationParams, GenerationConfig, create_sample, format_sample
