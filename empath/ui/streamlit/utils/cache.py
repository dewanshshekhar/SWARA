...
Creates / caches EmpathHandler and LLMHandler instances and exposes
...
@st.cache_resource
def get_dit_handler():
    """Return a cached EmpathHandler instance (uninitialised)."""
    try:
        from empath.handler import EmpathHandler
        logger.info("Creating EmpathHandler instance...")
        return EmpathHandler()
    except Exception as exc:
        logger.error(f"Failed to create EmpathHandler: {exc}")
        return None
...
def initialize_dit(
    config_path: str = "empath-v15-turbo",
    device: str = "auto",
    ...
    handler = get_dit_handler()
    if handler is None:
        return "EmpathHandler could not be created", False
...
def initialize_llm(
    lm_model_path: str = "empath-5Hz-lm-1.7B",
    backend: str = "mlx",
    device: str = "auto",
    offload_to_cpu: bool = False,
) -> Tuple[str, bool]:
