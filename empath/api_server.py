"""FastAPI server for Empath V1.5.
...
from empath.api.train_api_service import (
    initialize_training_state,
)
from empath.api.jobs.store import _JobStore
from empath.api.log_capture import install_log_capture
from empath.api.route_setup import configure_api_routes
from empath.api.server_cli import run_api_server_main
from empath.api.lifespan_runtime import initialize_lifespan_runtime
from empath.api.job_blocking_generation import run_blocking_generate
from empath.api.job_execution_runtime import run_one_job_runtime
from empath.api.job_model_selection import select_generation_handler
from empath.api.job_runtime_state import (
...
from empath.api.startup_model_init import initialize_models_at_startup
from empath.api.worker_runtime import start_worker_tasks, stop_worker_tasks
from empath.api.server_utils import (
...
from empath.api.http.auth import (
    set_api_key,
    verify_api_key,
    verify_token_from_request,
)
from empath.api.http.release_task_audio_paths import (
    save_upload_to_temp as _save_upload_to_temp,
    validate_audio_path as _validate_audio_path,
)
from empath.api.http.release_task_models import GenerateMusicRequest
from empath.api.http.release_task_param_parser import (
    RequestParser,
    _to_float as _request_to_float,
    _to_int as _request_to_int,
)
from empath.api.runtime_helpers import (
...
from empath.api.model_download import (
    ensure_model_downloaded as _ensure_model_downloaded,
)

from empath.handler import EmpathHandler
from empath.llm_inference import LLMHandler
from empath.constants import (
...
from empath.ui.gradio.events.results_handlers import _build_generation_info
...
RESULT_KEY_PREFIX = "empath_v1.5_"
...
def create_app() -> FastAPI:
    ...
    api_key = os.getenv("EMPATH_API_KEY", None)
    set_api_key(api_key)

    QUEUE_MAXSIZE = int(os.getenv("EMPATH_QUEUE_MAXSIZE", "200"))
    WORKER_COUNT = int(os.getenv("EMPATH_QUEUE_WORKERS", "1"))  # Single GPU recommended

    INITIAL_AVG_JOB_SECONDS = float(os.getenv("EMPATH_AVG_JOB_SECONDS", "5.0"))
    AVG_WINDOW = int(os.getenv("EMPATH_AVG_WINDOW", "50"))
    ...
    async def lifespan(app: FastAPI):
        ...
        async def _run_one_job(job_id: str, req: GenerateMusicRequest) -> None:
            ...
            def _build_blocking_result(
                selected_handler: EmpathHandler,
    ...
    app = FastAPI(title="Empath API", version="1.0", lifespan=lifespan)
