"""Unit tests for empath.training_v2.trainer_helpers.save_adapter_flat.
...
def _make_full_model(decoder: MagicMock) -> MagicMock:
    """Return a mock that looks like EmpathConditionGenerationModel.
...
class TestSaveAdapterFlatLora(unittest.TestCase):
    ...
    def test_calls_save_pretrained_on_peft_decoder_not_full_model(self):
        ...
        from empath.training_v2.trainer_helpers import save_adapter_flat
    ...
    def test_fallback_to_save_lora_weights_when_no_save_pretrained(self):
        ...
        with (
            patch("os.makedirs"),
            patch(
                "empath.training_v2.trainer_helpers.save_lora_weights"
            ) as mock_slw,
        ):
            save_adapter_flat(trainer, "/tmp/out")
    ...
    def test_regression_full_model_save_pretrained_not_called_for_lora(self):
        """Regression: the full Empath model's save_pretrained must never be called.

        Previously, _unwrap_decoder(module.model) returned the full
        EmpathConditionGenerationModel (which also has save_pretrained), causing
