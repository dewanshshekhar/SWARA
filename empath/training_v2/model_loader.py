"""
Lean Per-Phase Model Loading for Empath Training V2
...
_VARIANT_DIR = {
    "turbo": "empath-v15-turbo",
    "base": "empath-v15-base",
    "sft": "empath-v15-sft",
}
...
def _resolve_model_dir(checkpoint_dir: str | Path, variant: str) -> Path:
    ...
    """
    # 1. Known alias (turbo -> empath-v15-turbo, etc.)
...
def load_decoder_for_training(
    ...
    """Load the full ``EmpathConditionGenerationModel`` for training.
    ...
    Returns:
        The loaded ``EmpathConditionGenerationModel`` instance.
    """
...
def load_preprocessing_models(
    ...
    Returns a dict with keys:
        - ``model``: the full ``EmpathConditionGenerationModel``
...
def load_silence_latent(
    ...
    if sl_path is None and variant is not None:
        subdir = _VARIANT_DIR.get(variant, f"empath-v15-{variant}")
