"""
Empath: A Step Towards Music Generation Foundation Model
...
from empath.schedulers.scheduling_flow_match_heun_discrete import (
    FlowMatchHeunDiscreteScheduler,
)
from empath.schedulers.scheduling_flow_match_pingpong import (
...
from empath.language_segmentation import LangSegment, language_filters
from empath.music_dcae.music_dcae_pipeline import MusicDCAE
from empath.models.empath_transformer import EmpathTransformer2DModel
from empath.models.lyrics_utils.lyric_tokenizer import VoiceBpeTokenizer
from empath.apg_guidance import (
    apg_forward,
    MomentumBuffer,
    cfg_forward,
    cfg_zero_star,
...
# class EmpathPipeline(DiffusionPipeline):
class EmpathPipeline:
        ...
        if not checkpoint_dir:
            if persistent_storage_path is None:
                checkpoint_dir = os.path.join(
                    os.path.expanduser("~"), ".cache/empath/checkpoints"
                )
    ...
    def get_checkpoint_path(self, checkpoint_dir, repo):
        ...
        if checkpoint_dir is not None:
            required_dirs = ["music_dcae_f8c8", "music_vocoder", "empath_transformer", "umt5-base"]
            all_dirs_exist = True
    ...
    def load_checkpoint(self, checkpoint_dir=None, export_quantized_weights=False):
        ...
        empath_checkpoint_path = os.path.join(checkpoint_dir, "empath_transformer")
        text_encoder_checkpoint_path = os.path.join(checkpoint_dir, "umt5-base")

        self.empath_transformer = EmpathTransformer2DModel.from_pretrained(
            empath_checkpoint_path, torch_dtype=self.dtype
        )
        # self.empath_transformer.to(self.device).eval().to(self.dtype)
        if self.cpu_offload:
            self.empath_transformer = (
                self.empath_transformer.to("cpu").eval().to(self.dtype)
            )
        else:
            self.empath_transformer = (
                self.empath_transformer.to(self.device).eval().to(self.dtype)
        ...
        if self.torch_compile:
            self.empath_transformer = torch.compile(self.empath_transformer)
        ...
        if self.torch_compile:
            if export_quantized_weights:
                ...
                torch.save(
                    self.empath_transformer.state_dict(),
                    os.path.join(
                        empath_checkpoint_path, "diffusion_pytorch_model_int4wo.bin"
                    ),
                )
                print(
    ...
    def load_quantized_checkpoint(self, checkpoint_dir=None):
        ...
        self.empath_transformer = EmpathTransformer2DModel.from_pretrained(empath_checkpoint_path)
        self.empath_transformer.eval().to(self.dtype).to('cpu')
        self.empath_transformer = torch.compile(self.empath_transformer)
        self.empath_transformer.load_state_dict(
            torch.load(
                os.path.join(empath_checkpoint_path, "diffusion_pytorch_model_int4wo.bin"),
                map_location=self.device,
            ),assign=True
        )
        self.empath_transformer.torchao_quantized = True
    ...
    @cpu_offload("empath_transformer")
    def calc_v(
        self,
        zt_src,
        zt_tar,
        ...
        if return_src_pred:
            ...
            # source
            noise_pred_src = self.empath_transformer(
        ...
        # target
        noise_pred_tar = self.empath_transformer(
    ...
    def text2music_diffusion_process(
        ...
        def forward_encoder_with_temperature(self, inputs, tau=0.01, l_min=4, l_max=6):
            ...
            for i in range(l_min, l_max):
                handler = self.empath_transformer.lyric_encoder.encoders[
                    i
                ].self_attn.linear_q.register_forward_hook(hook)
                handlers.append(handler)

            encoder_hidden_states, encoder_hidden_mask = (
                self.empath_transformer.encode(**inputs)
            )
        ...
        encoder_hidden_states, encoder_hidden_mask = self.empath_transformer.encode(
            encoder_text_hidden_states,
            text_attention_mask,
            speaker_embds,
            lyric_token_ids,
        ...
        if use_erg_lyric:
            ...
            encoder_hidden_states_null, _ = self.empath_transformer.encode(
                torch.zeros_like(encoder_text_hidden_states),
                text_attention_mask,
        ...
        if do_double_condition_guidance:
            ...
            # P(null_speaker, text, no_lyric)
                ...
                encoder_hidden_states_no_lyric, _ = self.empath_transformer.encode(
                    encoder_text_hidden_states,
                    text_attention_mask,
                    torch.zeros_like(speaker_embds),
                    torch.zeros_like(lyric_token_ids),
        ...
        def forward_diffusion_with_temperature(
            ...
            for i in range(l_min, l_max):
                handler = self.empath_transformer.transformer_blocks[
                    i
            ...
            sample = self.empath_transformer.decode(
        ...
        for i, t in tqdm(enumerate(timesteps), total=num_inference_steps):
            ...
            if is_in_guidance_interval and do_classifier_free_guidance:
                ...
                noise_pred_with_cond = self.empath_transformer.decode(
                ...
                if (
                    ...
                    noise_pred_with_only_text_cond = self.empath_transformer.decode(
                        hidden_states=latent_model_input,
                        attention_mask=attention_mask,
                ...
                if use_erg_diffusion:
                    ...
                    noise_pred_uncond = self.empath_transformer.decode(
                        hidden_states=latent_model_input,
                        attention_mask=attention_mask,
                        encoder_hidden_states=encoder_hidden_states_null,
                        encoder_hidden_mask=encoder_hidden_mask,
                ...
                latent_model_input = latents
                timestep = t.expand(latent_model_input.shape[0])
                noise_pred = self.empath_transformer.decode(
                    hidden_states=latent_model_input,
                    attention_mask=attention_mask,
                    encoder_hidden_states=encoder_hidden_states,
                    encoder_hidden_mask=encoder_hidden_mask,
    ...
    def load_lora(self, lora_name_or_path, lora_weight):
        if (lora_name_or_path != self.lora_path or lora_weight != self.lora_weight) and lora_name_or_path != "none":
            ...
            if self.lora_path != "none":
                self.empath_transformer.unload_lora()
            self.empath_transformer.load_lora_adapter(os.path.join(lora_download_path, "pytorch_lora_weights.safetensors"), adapter_name="empath_lora", with_alpha=True, prefix=None)
            logger.info(f"Loading lora weights from: {lora_name_or_path} download path is: {lora_download_path} weight: {lora_weight}")
            set_weights_and_activate_adapters(self.empath_transformer, ["empath_lora"], [lora_weight])
