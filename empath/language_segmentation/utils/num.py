...
RE_RANGE = re.compile(
    r"""
    (?<![\d\+\-\×÷=])      # Use negative lookbehind to ensure there are no other numbers or operators before the number range
    ((-?)((\d+)(\.\d+)?))  # Match the starting negative or positive number (integer or decimal) of the range
    [-~]                   # Match range separator
    ((-?)((\d+)(\.\d+)?))  # Match the ending negative or positive number (integer or decimal) of the range
    (?![\d\+\-\×÷=])       # Use positive lookahead to ensure there are no other numbers or operators after the number range
    """,
    re.VERBOSE,
)
