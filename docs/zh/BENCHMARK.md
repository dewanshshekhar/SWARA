# Empath 1.5 Benchmark & Profiling Guide

**Language / Language:** [English](BENCHMARK.md) | [中文](../zh/BENCHMARK.md)
...
`profile_inference.py` is a comprehensive profiling & benchmarking tool for Empath 1.5 inference. It measures end-to-end wall time, LLM planning time, DiT diffusion time, VAE decoding time, and more — across different devices, backends, and configurations.
...
Automatically simulates different GPU VRAM sizes using `MAX_CUDA_VRAM` and runs a generation test at each tier. This is the recommended way to validate that all GPU tiers work correctly after modifying `empath/gpu_config.py`.
...
| Flag | Default | Description |
|------|---------|-------------|
| `--config-path` | `empath-v15-turbo` | DiT model config |
| `--lm-model` | `empath-5Hz-lm-1.7B` | LLM model path |
...
```bash
# Lightweight (0.6B)
python profile_inference.py --lm-model empath-5Hz-lm-0.6B

# Default (1.7B)
python profile_inference.py --lm-model empath-5Hz-lm-1.7B

# Large (4B)
python profile_inference.py --lm-model empath-5Hz-lm-4B
```
...
python profile_inference.py --offload-to-cpu --quantization int8_weight_only --lm-model empath-5Hz-lm-0.6B
```
...
5. **GPU memory auto-adaptation** — The benchmark mode automatically clamps durations and batch sizes to what your GPU can handle, using the adaptive tier system in `empath/gpu_config.py`.
